...
function hashString(s) {
  // Bun (native install): use Bun.hash for correctness
  if (typeof Bun !== 'undefined') {
    return Number(BigInt(Bun.hash(s)) & 0xffffffffn)
  }
  // Node.js (npm install): FNV-1a fallback
  let h = 2166136261
...
function validateOptions(options) {
  ...
  if (options.query) return // query mode skips filter validation
...
Usage:
  node find-buddy.js --species ghost --rarity legendary --shiny
  bun  find-buddy.js --species cat --rarity legendary --count 3

Compatible with both Node.js (npm install) and Bun (native install).
...
Options:
  ...
  --query <userId>      Query what pet a specific userId rolls (no search)
  --help                Show this help
`)
}
...
function main() {
  ...
  // Query mode: show what pet a specific userId rolls
  if (options.query) {
    const { bones, inspirationSeed } = rollUserId(options.query)
    console.log(JSON.stringify({ userId: options.query, bones, inspirationSeed }, null, 2))
    return
  }
